...
const pool = new Pool({
    ...
    password: (() => {
        if (!process.env.DB_PASSWORD) {
            ...
            console.error('   Varsayılan şifre kaldırıldı — güvenlik gereği tanımlanmalı.');
            process.exit(1);
        }
...
app.post('/api/auth/register', validateAuthInput, upload.single('profilePic'), async (req, res) => {
    try {
        ...
        if (existingEmail) return res.status(400).json({ error: 'Bu bilgilerle kayıt oluşturulamadı' });
